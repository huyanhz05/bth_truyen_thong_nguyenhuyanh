...
// Giúp tránh cập nhật LCD liên tục khi không cần thiết
byte lastSecondDisplayed = 60; // Khởi tạo giá trị không hợp lệ (0-59)
                               // để buộc cập nhật lần đầu tiên
  ...
  if (modeState) { // Nút đang được nhấn
    if (modePressStartTime == 0) { // Mới nhấn
      ...
      if (currentMode == DISPLAY_TIME) {
        ...
        // <<< THAY ĐỔI (2) >>>
        // Buộc màn hình cập nhật lại trạng thái "ON"/"OFF" ngay lập tức
...
void updateDisplay() {
  ...
  if (millis() - lastBlinkTime > 500) {
    ...
    lastBlinkTime = millis(); // <<< THAY ĐỔI (3): Sửa lỗi logic (trước đó là 0)
  }
  ...
  if (currentMode != lastMode) {
    ...
    // <<< THAY ĐỔI (4) >>>
    // Buộc cập nhật màn hình khi quay lại màn hình chính
    lastSecondDisplayed = 60; 
  }
  ...
  switch (currentMode) {
    case DISPLAY_TIME:
      // <<< THAY ĐỔI (5) - THAY ĐỔI QUAN TRỌNG NHẤT >>>
      // Chỉ cập nhật màn hình NẾU giây đã thay đổi
      if (now.second() != lastSecondDisplayed) {
        lastSecondDisplayed = now.second(); // Lưu lại giây vừa hiển thị
        ...
        lcd.print(" "); // Thêm khoảng trắng để ghi đè (vd: 10:00:09 -> 10:00:10)
